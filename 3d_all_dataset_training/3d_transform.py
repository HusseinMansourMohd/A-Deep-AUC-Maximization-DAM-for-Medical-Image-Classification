...
class RandomRotation3D:
    ...
    def __call__(self, x):
        ...
        x_np = x.numpy()
        rotated_x = rotate(x_np, angle, axes=axes, reshape=False, mode='nearest')
        return torch.from_numpy(rotated_x)
...
augmentations = Compose([RandomRotation3D(degrees=20), RandomFlip3D(p=0.5)])
...
for epoch in range(NUM_EPOCHS):
    ...
    for inputs, targets in train_loader:
        ...
        inputs = augmentations(inputs.cpu())
    ...
    if test_accuracy > best_test_accuracy:
        ...
        torch.save(model.state_dict(), '3d_data_augmentation_best_model_16_batch_size.pth')
