...
best_hyperparameters = {
    ...
    'learning_rate': 0.1,  # Replace with the best learning rate
    'momentum': 0.9,  # Replace with the best momentum value
    'loss_function': 'CrossEntropyLoss'  # Replace with the best loss function name
}
#{'optimizer': 'Adam', 'learning_rate': 0.1, 'momentum': 0.9, 'loss_function': 'CrossEntropyLoss'}
num_epochs = 100
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        if val_auc > best_auc:
            best_auc = val_auc
            torch.save(model.state_dict(), 'trained_synapsemnist3d_model_with_augmentation_best_auc.pth')
